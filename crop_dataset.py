...
PATH = "/Users/ntdat/Downloads/DB_data"
SAVE = "/Users/ntdat/Downloads/DB_data_Landmarks"

for dir in os.listdir(PATH):
    path_to_dir = os.path.join(PATH, dir)
    path_to_save_dir = os.path.join(SAVE, dir)
    if not os.path.exists(path_to_save_dir):
        os.mkdir(path_to_save_dir)
    for img_name in os.listdir(path_to_dir):
        face = cv2.imread(os.path.join(path_to_dir, img_name))
        w, h, c = face.shape
        ...
        cv2.imwrite(os.path.join(path_to_save_dir, img_name), _face)
